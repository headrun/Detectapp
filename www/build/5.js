...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__newpost__ = __webpack_require__(323);
    ...
    NewpostPage.prototype.publishNewPost = function () {
        ...
        if (imgfile != '' && imgfile != undefined) {
            ...
            this.saveNewPostImgData(imgfile, function (imgUrl) {
                var postObj = { 'comments_count': 0, 'created_at': new Date().getTime(), 'is_abuse': false, 'is_active': true, 'is_pinned': 0, 'like_users': [''], 'likes_count': 0, 'modified_at': new Date().getTime(), 'post': newpost, 'post_image': imgUrl, 'posted_by': _this.phoneNumber };
                console.log(postObj);
            ...
            this.lctr.present();
            var postObj = { 'comments_count': 0, 'created_at': new Date().getTime(), 'is_abuse': false, 'is_active': true, 'is_pinned': 0, 'like_users': [''], 'modified_at': new Date().getTime(), 'likes_count': 0, 'post': newpost, 'post_image': '', 'posted_by': this.phoneNumber };
            __WEBPACK_IMPORTED_MODULE_2_firebase__["database"]().ref(groupid + '_posts').push(postObj).then(function (res) {
                var tctrl = _this.toastCtrl.create({
                    message: 'Posted Successfully.',
                    duration: 2000,
