...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__group__ = __webpack_require__(318);
...
var GroupPage = /** @class */ (function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, __WEBPACK_IMPORTED_MODULE_2_firebase__["database"]().ref('userDetails').orderByChild('mobileNo').equalTo(this.phoneNumber).on('child_added', function (datasnapshot) {
                                if (datasnapshot.val().group_id != undefined && datasnapshot.val().group_id !== '') {
                                    var gids = datasnapshot.val().group_id;
                                    _this.getExtraGroups_Filter(gids, callback);
                                }
                                else {
                                    callback([]);
                                }
    ...
    GroupPage.prototype.getExtraGroups_Filter = function (userGroupids, callback) {
        var _this = this;
        var total_groups = [];
        var total_g_ids = [];
        var final_sorted_groups = [];
        __WEBPACK_IMPORTED_MODULE_2_firebase__["database"]().ref('groups').once('value', function (datasnapshot) {
            datasnapshot.forEach(function (childsnapshot) {
                var g_obj = childsnapshot.val();
                total_g_ids.push(childsnapshot.key);
                g_obj['id'] = childsnapshot.key;
                total_groups.push(g_obj);
            });
            for (var j = 0; j < total_g_ids.length; j++) {
                var index = total_g_ids.indexOf(userGroupids[j]);
                if (index != -1) {
    ...
    GroupPage.prototype.joinGroup = function (index) {
        var _this = this;
        console.log(this.phoneNumber);
        var group = this.userGroupsData[index];
        __WEBPACK_IMPORTED_MODULE_2_firebase__["database"]().ref('userDetails').orderByChild('mobileNo').equalTo(this.phoneNumber).once('value', function (datasnapshot) {
            datasnapshot.forEach(function (userObject) {
                var u_obj = userObject.val();
                if (u_obj.hasOwnProperty('group_id')) {
                    u_obj['group_id'].push(group['id']);
                    console.log(u_obj);
                    __WEBPACK_IMPORTED_MODULE_2_firebase__["database"]().ref('userDetails/' + userObject.key).set(u_obj).then(function (_) {
                        _this.userGroupsData[index]['joined'] = true;
